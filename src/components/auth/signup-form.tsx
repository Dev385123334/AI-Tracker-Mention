...
type SignupState = {
  error?: { name?: string[]; email?: string[]; password?: string[]; confirmPassword?: string[]; form?: string[] }
}

export function SignupForm() {
  const wrappedAction = async (_prev: SignupState | undefined, formData: FormData): Promise<SignupState | undefined> =>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1.5">
        <h1 className="text-[22px] font-bold tracking-[-0.02em] text-[#111827]">Create your account</h1>
        <p className="text-sm text-[#6B7280]">Start tracking your brand&apos;s AI visibility</p>
      </div>

      <form action={formAction} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="name" className="text-xs font-medium text-[#111827]">Name</Label>
          <Input
            id="name"
            name="name"
          ...
          />
          {state?.error?.name && (
            <p className="text-xs text-destructive mt-1">{state.error.name[0]}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="password" className="text-xs font-medium text-[#111827]">Password</Label>
          <Input
            id="password"
            name="password"
            type="password"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="confirmPassword" className="text-xs font-medium text-[#111827]">Confirm Password</Label>
          <Input
            id="confirmPassword"
          ...
          {state?.error?.confirmPassword && (
            <p className="text-xs text-destructive mt-1">{state.error.confirmPassword[0]}</p>
          )}
        </div>
      ...
      <Button
        variant="outline"
        className="w-full h-11 rounded-xl border-[#E5E7EB] text-[#111827] hover:bg-[#F7F8FC] text-sm font-medium"
      >
